...
>(({ defaultOpen = true, open: openProp, onOpenChange, className, style, children, ...props }, ref) => {
  ...
  // We use openProp and onOpenChange for external control.
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0} skipDelayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex min-h-svh w-full has-[[data-variant=inset]]:bg-sidebar relative gap-0",
            className
          )}
          ref={ref}
          {...props}
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "duration-200 relative h-svh bg-transparent transition-[width] ease-linear shrink-0 flex",
        ...
        >
          <div
            className={cn(
              ...
              variant === "floating" || variant === "inset"
                ? "group-data-[side=left]:left-4 group-data-[side=right]:right-4 group-data-[collapsible=icon]:w-[var(--sidebar-width-icon)]"
                : ""
            )}
          >
            <div
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <main
        ref={ref}
        className={cn(
          "relative flex min-h-svh flex-1 flex-col bg-background min-w-0 transition-[margin-left] duration-200 ease-linear !border-l-0 !p-0 !m-0",
        ...
        style={{
          marginLeft: isCollapsed ? "var(--sidebar-width-icon)" : "var(--sidebar-width)",
          padding: 0,
          marginTop: 0,
          marginRight: 0,
          marginBottom: 0,
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn("mx-2 w-full shrink-0 bg-sidebar-border h-px", className)}
      {...props}
    />
  )
})
  ...
  ({ className, ...props }, ref) => {
    return (
      <ul
        ref={ref}
        data-sidebar="menu"
        className={cn("flex w-full min-w-0 flex-col gap-0.5 group-data-[collapsible=icon]:gap-1", className)}
        {...props}
      />
    )
  }
...
>(
  (
    ...
    if (asChild) {
      return (
        <Slot data-sidebar="menu-button" data-active={isActive} data-size={size}>
          {button}
        </Slot>
      )
    }
...
>(({ asChild = false, showOnHover = false, className, ...props }, ref) => {
  ...
  if (asChild) {
    return <Slot data-sidebar="menu-action">{button}</Slot>
  }
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  if (asChild) {
    return <Slot data-sidebar="menu-sub-button">{button}</Slot>
  }
